...
if (isset($_POST["email"], $_POST["password"])) {
    ...
    //requete prepare pour joueur//
    $sqlJoueur = "SELECT * FROM joueur WHERE email = ? AND password = ?";
    $queryJoueur = $connect->prepare($sqlJoueur);
    $queryJoueur->execute([$email, $password]);

   
    if ($queryJoueur->rowCount() === 1) {
        //test pour joueur
        header("Location: menu_joueur.php");
        exit;
    } else {
...
<body >
    ...
    <div class="input">
        ...
        <input type="submit" name="login" value="S'identifier" class="login"></input><br>
         <h4><a href="inscription.php">creer un compte </a>!</h4>
    </form>
    </div>
  </div>
   <!-- pied de page -->
